...
\begin{figure}[htbp]
  % \includegraphics[width=\columnwidth]{assets/overcommit/exec-sec.png}
  \todo{include}
...
When spawning a single fuzzing client per core, as is usual in fuzzing, the CPU averages approximately 0.8\% user space time and 1.9 system space time. Compared to a single client per core, using an \code{overcommit} value of 10 (thus spawning 10 clients per core) does not result in increased inconsistencies for both coverage and state-diff feedback. However, when using 20 clients per core, this no longer holds, as demonstrated in \cref{fig:overcommit-consistencies}. The CPU loads for \code{overcommit} values of 10 and 20 are \todo{measure} respectively.

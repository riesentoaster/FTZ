...
As Zephyr provides built-in functionality in \code{native_sim} to run it natively on a Linux host (see \cref{Implementation:native_sim}), any improvements regarding emulation techniques are considered out of scope. Furthermore, \code{native_sim} provides the required mapping of hardware from host to Zephyr and \proj introduces custom logic to exchange networking packets between the fuzzer and PUT, as described in \cref{Implementation:NetworkInterface}, thus negating the need for additional environment modelling.

It is worth noting that Zephyr provides a pseudo-target similar to \code{native_sim}, which allows running it in QEMU. Because NativeSimulator, which provides the basis for \code{native_sim}, was specifically built to be faster than its emulation counterpart\cite{NativeSimulator}, this was not pursued further. There exist however examples of fuzzers using Zephyr's QEMU such as para-rehosting\cite{ParaRehosting}.

Refer to the various dedicated review papers\cite{EmbeddedReview,IoTSurvey,ChallengesReHosting,SoK} for additional details on generic techniques to fuzz embedded OSs such as advancements in re-hosting and modelling of memory-mapped IO devices.
...
% Done:
% \begin{itemize}
%   \item \citetitle{NetworkBugsReview}\cite{NetworkBugsReview}: Review of bugs found in embedded network stacks, Generational fuzzer
%   \item \citetitle{EnhanceVP}\cite{EnhanceVP}: TCP/IP of Zephyr, called SISSI, \textquote{strongly enhance all major aspects of a modern VP[Virtual Prototype]-based design flow}.
%   \item \citetitle{MultiFuzz}\cite{MultiFuzz}: multi-stream input generation and representation, better than Fuzzware and Ember-IO, contains comparison to Hoedur. Zephyr SocketCan. Ghidra Emulator.
%   \item \citetitle{SplITS}\cite{SplITS}: Fuzzware. state? multistream
%   \item \citetitle{ESFuzz}\cite{ESFuzz}: Improve MMIO models with state
%   \item \citetitle{Fuzzware}\cite{Fuzzware}: Network of Zephyr, found bugs.QEMU
%   \item \citetitle{EmberIO}\cite{EmberIO}: Zephyr socketCAN, Model-Free Memory Mapped. QEMU
%   \item \citetitle{Esoteric}\cite{Esoteric}: LibAFL, MetaEmu, Ghidra Emulator
%   \item \citetitle{ParaRehosting}\cite{ParaRehosting}: QEMU, uses Zephyr-QEMU
%   \item \citetitle{Veri}\cite{Veri}: IP of Zephyr, using the provided Serial Line Internet Protocol (SLIP) to pass UDP messages. libFuzzer. (VP-CFG)
%   \item \citetitle{Tardis}\cite{Tardis}: QEMU
%   \item \citetitle{HDFuzz}\cite{HDFuzz}: Uses symbex to build models of memory-mapped IO, QEMU. Read more extensively.
%   \item \citetitle{Hoedur}\cite{Hoedur}: Recent advancements in emulation techniques, not in fuzzing. They propose multi-stream input including types, with proper mutation. Other fuzzing evaluation uses off-the-shelf fuzzers with \textquote{flat, sequential, and opaque firmware fuzzing input} models. QEMU.
%   \item \citetitle{PATA}\cite{PATA}: Finds Zephyr IP bug
% \end{itemize}
